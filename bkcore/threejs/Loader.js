...
bkcore.threejs.Loader.prototype.load = function (data) {
  ...
  for (var k in this.types) {
    if (k in data) {
      var size = 0;
      for (var j in data[k]) size++;
      this.progress.total += size;
      this.progress.remaining += size;
    }
  }

  for (var t in data.textures) this.loadTexture(t, data.textures[t]);

  for (var c in data.texturesCube)
    this.loadTextureCube(c, data.texturesCube[c]);

  for (var g in data.geometries) this.loadGeometry(g, data.geometries[g]);

  for (var a in data.analysers) this.loadAnalyser(a, data.analysers[a]);

  for (var i in data.images) this.loadImage(i, data.images[i]);
...
bkcore.threejs.Loader.prototype.loadGeometry = function (name, url) {
  ...
  this.jsonLoader.load(url, function (a) {
...
bkcore.threejs.Loader.prototype.loadAnalyser = function (name, url) {
  ...
  this.data.analysers[name] = new bkcore.ImageData(url, function () {
